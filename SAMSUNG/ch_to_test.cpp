...
int main() {
  ...
  for (int query = 1; query <= q; ++query) {
    ...
    for (GROUP &now : groups) {
      ...
      for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
          temp_map[i - sy][j - sx] = (now.map[i][j]) ? now.map[i][j] : 0;
        }
      }
      for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
          now.map[i][j] = temp_map[i][j];
        }
      }
        ...
        for (int i = 0; i < n; ++i) {
          ...
          for (int row = 0; row < n; ++row) {
            ...
            for (int col = 0; col < n; ++col) {
              if (!now.map[row][col]) continue;
              int ny = i + row;
              int nx = j + col;
              if (ny < 0 || nx < 0 || ny >= n || nx >= n) {
                // if (query == 37) {
                //   cout << "\ni = " << i << ", j = " << j << "\nny = " << ny << ", nx = " << nx << " and passed because out of bounds\n";
                // }
                good_to_go = false;
                break;
              }
              ...
              if (MAP[ny][nx]) {
                // if (query == 37) {
                //   cout << "\ni = " << i << ", j = " << j << "\nny = " << ny << ", nx = " << nx << " and passed because MAP overlap\n";
                // }
                good_to_go = false;
                break;
              }
