...
struct GROUP
{
  ...
  int map[MAX_SIZE][MAX_SIZE]; // sy, sx 를 기준으로 어느 위치에 미생물이 있는지 (즉 모양을 결정) -> 값은 num 과 같음
  bool isAlive = true;
...
int bfs(int y, int x, int map[MAX_SIZE][MAX_SIZE])
{
  ...
  while (!q.empty())
  {
    ...
    for (int i = 0; i < 4; ++i)
    {
      int ny = now.y + dy[i];
      int nx = now.x + dx[i];
      if (ny < 0 || nx < 0 || ny >= MAX_SIZE || nx >= MAX_SIZE)
        continue;
      if (visited[ny][nx])
        continue;
      if (!map[ny][nx])
...
int main() 
{
  ...
  for (int query = 1; query <= q; ++query)
  {
    ...
    cout << "MAP after putting in GROUP\n";
    for (int i = 0; i < n; ++i) {
      for (int j = 0; j < n; ++j) {
        cout << MAP[i][j] << ' ';
      }
    ...
    {
      int group_num = sorted_groups.top().num; sorted_groups.pop();
        ...
        for (int j = 0; j < n; ++j)
        {
          ...
          for (int row = 0; row < n; ++row)
          {
            ...
            for (int col = 0; col < n; ++col)
            {
              int ny = i + row;
              int nx = j + col;
              if ((ny < 0 || nx < 0 || ny >= n || nx >= n || MAP[ny][nx]) && now.map[row][col]) {
                good_to_go = false;
                break;
              }
            }
